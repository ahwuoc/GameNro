    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            switch (this.mapId) {
                ...
                case 181 -> {
                    List<String> menu = new ArrayList<>(Arrays.asList("Quay về"));
                    if (InventoryService.gI().findItem(player, 921) || InventoryService.gI().findItem(player, 1604)
                            || InventoryService.gI().findItem(player, 1605)) {
                        menu = new ArrayList<>(Arrays.asList("Quay về", "Mở chỉ số\nBông tai\nPorata cấp\n3",
                                "Nâng cấp\nBông tai\nPorata3", "Hắc Hóa\nPorata Cấp\n3"));
                    }

                    String[] menus = menu.toArray(new String[0]);
                    this.createOtherMenu(player, ConstNpc.BASE_MENU, "Cũng Khá lắm mới tới được đây e hèm??", menus);
                }
                default -> {
                            ...
                            switch (select) {
                                case 0 ->
                                    createOtherMenu(player, ConstMenu.MENU_PHA_LE,
                                            "Ta sẽ giúp ngươi nâng cấp trang bị\n"
                                                    + "\b|5|Số lần may mắn hiện tại: " + player.point_lucky_dapdo,
                                            "Ép sao\ntrang bị", "Pha lê\nhoá\ntrang bị");
                                ...
                                case 5 ->
                                    CombineService.gI().openTabCombine(player, CombineService.PHAN_RA_DO_TL);
                                case 4 ->
                                    createOtherMenu(player, ConstMenu.CHUC_NANG_BHM_KHAC, "Ngươi tìm ta có việc gì?\n",
                                            "Pháp sư hoá", "Tẩy Pháp sư hóa", "Tinh ân");
                                case 2 ->
                        ...
                        case ConstNpc.MENU_START_COMBINE -> {
                            switch (player.combine.typeCombine) {
                                ...
                                case CombineService.NANG_CAP_KICH_HOAT_VIP,
                                        ...
                                        CombineService.NANG_CAP_BONG_TAI2,
                    ...
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                            case 0: {
                                ...
                                break;
                            case 1: {
                                createOtherMenu(player, ConstNpc.MENU_OPTION_BONGTAI, "Nâng bông tai nào ?",
                                        "Đổi\n Chỉ số\n Dòng 1", "Đổi\n Chỉ số\n Dòng 2");
                            }
                                break;
                            case 2: {
                                CombineService.gI().openTabCombine(player, CombineService.NANG_CAP_BONG_TAI2);
                            }
                                break;
                            case 3: {
                                createOtherMenu(player, ConstNpc.MENU_HACHOA_PORATA,
                                        "Hắc hóa porata giúp cường hóa skill khi hợp thể\n"
                                                + "\b|5|Trái đất sẽ cường hóa skill kamejoko \n"
                                                + "\b|5|Xayda cường hóa skill Khỉ\n"
                                                + "\b|5|Namec cường hóa skill Liên hoàn",
                                        "Hắc Hóa Ngay", "Đóng");
                            }
                                break;
                        ...
                        switch (player.combine.typeCombine) {
                            case CombineService.NANG_CAP_BONG_TAI2, CombineService.MO_CHI_SO_OPTION1,
                                    CombineService.MO_CHI_SO_OPTION2, CombineService.HAC_HOA_PORATA -> {
                                if (select == 0) {
                                    CombineService.gI().startCombine(player);
                                }
                            }
                        }
                    } else if (player.iDMark.getIndexMenu() == ConstNpc.MENU_OPTION_BONGTAI) {
                        switch (select) {
                            case 0: {
                                CombineService.gI().openTabCombine(player, CombineService.MO_CHI_SO_OPTION1);
                            }
                                break;
                            case 1: {
                                CombineService.gI().openTabCombine(player, CombineService.MO_CHI_SO_OPTION2);
                            }
                                break;
                        }
                    } else if (player.iDMark.getIndexMenu() == ConstNpc.MENU_HACHOA_PORATA) {
                        switch (select) {
                            case 0: {
                                CombineService.gI().openTabCombine(player, CombineService.HAC_HOA_PORATA);
                            }
                                break;
