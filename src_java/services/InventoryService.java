...
public class InventoryService {
    ...
    private Item putItemBody(Player player, Item item) {
        ...
        if (player.isDeTu) {
            if (item.template.id != 1230 && item.template.id != 1489) {
                switch (item.template.type) {
                    ...
                    case 32:
                        break;
                    default:
                        Service.gI().sendThongBaoOK(((Pet) player).master, "Trang bị không phù hợp!");
                        return sItem;
                }
            }
        } else {
            switch (item.template.type) {
                ...
                case 97:
                    break;
                default:
                    Service.gI().sendThongBaoOK(player, "Trang bị không phù hợp!");
                    return sItem;
            }
        }
        if (item.template.gender < 3 && item.template.gender != player.gender) {
            Service.gI().sendThongBaoOK(player.isDeTu ? ((Pet) player).master : player, "Trang bị không phù hợp!");
            return sItem;
        }
        ...
        if (player.nPoint.power < powerRequire) {
            Service.gI().sendThongBaoOK(player.isDeTu ? ((Pet) player).master : player, "Sức mạnh không đủ yêu cầu!");
            return sItem;
        }
        int index = -1;
        switch (item.template.type) {
        ...
        if (item.template.id == 1230 && player.isDeTu) {
            index = 7;

        }
        if (item.template.id == 1489 && player.isDeTu) {
            index = 8;
        }
        sItem = player.inventory.itemsBody.get(index);
        if (sItem.isNotNullItem() && player.isDeTu && sItem.template.id == 1489 && index == 8) {
            Service.gI().sendPetFollow(player, (short) 0);
        }
        if (index == 8 || index == 10 || index == 13 || index == 12) {
            if (sItem.isNotNullItem() && sItem.template.id != 1489 && sItem.template.id != 1230) {
    ...
    public void itemBodyToBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            if (index == 10) {
                int id = item.template.part;
                Service.gI().sendTitle(player, id);
            }
            ...
            sendItemBag(player);
            sendItemBody(player);
            if (index == 8 || index == 10 || index == 12 || index == 7) {
                if (item.isNotNullItem()) {
                    if (index == 8 && player.isDeTu && item.template.id == 1489) {
                        Service.gI().sendPetFollow(player, (short) 0);
                    }
                    if (index == 7 && player.isDeTu && item.template.id == 1230) {
    ...
    public boolean addItemBag(Player player, Item item) {
        return addItemBag(player, item, "Không rõ nguồn");
    }

    public boolean addItemBag(Player player, Item item, String source) {
        // ngọc rồng đen
        ...
        int qtyBefore = item.quantity;
        boolean success = addItemList(player.inventory.itemsBag, item);
        if (success && item.template.id == 1788) {
            jdbc.daos.LogItem1788DAO.log(player, source, qtyBefore);
        }
        return success;
    }
    ...
    public boolean findItemBongTaiCap3(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 1604) {
                return true;
            }
        }
        for (Item item : player.inventory.itemsBox) {
