...
//                Service.gI().sendThongBao(plKicked, "Bạn đã bị đuổi khỏi bang");
    ...
    public void close() {
        ...
        try ( Connection con = DBConnecter.getConnectionServer();) {
            ps = con.prepareStatement("update clan set slogan = ?, img_id = ?, power_point = ?, max_member = ?, clan_point = ?, level = ?, members = ?, name_2 = ?, tops = ?,Point=? where id = ? limit 1");
            for (Clan clan : Manager.CLANS) {
