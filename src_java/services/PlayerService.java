...
public class PlayerService {
    ...
    public void sendTNSM(Player player, byte type, long param) {
        if (param > 0) {
            ...
            try {
                ...
                // msg.writeLongByDucPro(param, cn.readInt);// số tn cần cộng
    ...
    public void sendInfoHp(Player player) {
        Message msg;
        try {
            msg = Service.gI().messageSubCommand30((byte) 5);
            msg.writeLongByDucPro(Util.toIntOrLong(player.nPoint.hp), cn.readInt);
            player.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Logger.logException(PlayerService.class, e);
    ...
    public void sendInfoMp(Player player) {
        Message msg;
        try {
            msg = Service.gI().messageSubCommand30((byte) 6);
            msg.writeLongByDucPro(Util.toIntOrLong(player.nPoint.mp), cn.readInt);
            player.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Logger.logException(PlayerService.class, e);
    ...
    public void sendInfoHpMpMoney(Player player) {
        ...
        Message msg;
        try {
            msg = Service.gI().messageSubCommand30((byte) 4);
            try {
            ...
            msg.writeLongByDucPro(Util.toIntOrLong(player.nPoint.hp), cn.readInt);// chp
            msg.writeLongByDucPro(Util.toIntOrLong(player.nPoint.mp), cn.readInt);// cmp
