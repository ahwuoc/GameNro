 ...
 * @author Admin
 */
public class Tophopdiem {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophopdiem INSTANCE = new Tophopdiem();

    public static Tophopdiem getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hopdiem, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hopdiem IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hopdiem = rs.getInt("hopdiem");
