 ...
 * @author Admin
 */
public class Topthiep_halloween {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topthiep_halloween INSTANCE = new Topthiep_halloween();

    public static Topthiep_halloween getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.thiep_halloween, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.thiep_halloween IS NOT NULL " +
             ...
             "ORDER BY a.thiep_halloween DESC " +
             "LIMIT 100";
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.thiep_halloween = rs.getInt("thiep_halloween");
