 ...
 * @author Admin
 */
public class Tophopquathang9 {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophopquathang9 INSTANCE = new Tophopquathang9();

    public static Tophopquathang9 getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hopquathang9, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hopquathang9 IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hopquathang9 = rs.getInt("hopquathang9");
