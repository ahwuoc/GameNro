...
public class TopKhiGas {
    ...
    public void load() {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichKhiGas, ',', 1), '[', -1) AS UNSIGNED) AS part1, "
                + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichKhiGas, ',', -1), ']', 1) AS UNSIGNED) AS part2 "
                + "FROM clan "
