 ...
 * @author Admin
 */
public class Tophalloween_master {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophalloween_master INSTANCE = new Tophalloween_master();

    public static Tophalloween_master getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.halloween_master, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.halloween_master IS NOT NULL " +
             ...
             "ORDER BY a.halloween_master DESC " +
             "LIMIT 100";
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.halloween_master = rs.getInt("halloween_master");
