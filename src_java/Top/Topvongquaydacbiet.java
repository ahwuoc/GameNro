 ...
 * @author Admin
 */
public class Topvongquaydacbiet {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topvongquaydacbiet INSTANCE = new Topvongquaydacbiet();

    public static Topvongquaydacbiet getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.vongquaydacbiet, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.vongquaydacbiet IS NOT NULL " +
             ...
             "ORDER BY a.vongquaydacbiet DESC " +
             "LIMIT 100";
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.vongquaydacbiet = rs.getInt("vongquaydacbiet");
