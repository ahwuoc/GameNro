 ...
 * @author Admin
 */
public class Topphaobong {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topphaobong INSTANCE = new Topphaobong();

    public static Topphaobong getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.phaobong, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.phaobong IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.phaobong = rs.getInt("phaobong");
    ...
    private void extractDataPoint(String dataPoint, Player player) {
        ...
        player.nPoint.power = toLong(dataArray.get(11));
    }
