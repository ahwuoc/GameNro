 ...
 * @author Admin
 */
public class Tophopquatrungthuvip {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophopquatrungthuvip INSTANCE = new Tophopquatrungthuvip();

    public static Tophopquatrungthuvip getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hopquatrungthuvip, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hopquatrungthuvip IS NOT NULL " +
             ...
             "ORDER BY a.hopquatrungthuvip DESC " +
             "LIMIT 100";
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hopquatrungthuvip = rs.getInt("hopquatrungthuvip");
