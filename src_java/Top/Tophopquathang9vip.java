 ...
 * @author Admin
 */
public class Tophopquathang9vip {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophopquathang9vip INSTANCE = new Tophopquathang9vip();

    public static Tophopquathang9vip getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hopquathang9vip, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hopquathang9vip IS NOT NULL " +
             ...
             "ORDER BY a.hopquathang9vip DESC " +
             "LIMIT 100";
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hopquathang9vip = rs.getInt("hopquathang9vip");
