 ...
 * @author Admin
 */
public class Topthiepchucvip {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topthiepchucvip INSTANCE = new Topthiepchucvip();

    public static Topthiepchucvip getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.thiepchucvip, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.thiepchucvip IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.thiepchucvip = rs.getInt("thiepchucvip");
