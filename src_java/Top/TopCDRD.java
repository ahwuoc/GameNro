 ...
 */
public class TopCDRD {

    @Getter
    private List<Clan> list = new ArrayList<>();
    
    private static final TopCDRD INSTANCE = new TopCDRD();
    

    public static TopCDRD getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, ',', 1), '[', -1) AS UNSIGNED) AS part1, "
                + "CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(thanhTichCDRD, ',', -1), ']', 1) AS UNSIGNED) AS part2 "
                + "FROM clan "
