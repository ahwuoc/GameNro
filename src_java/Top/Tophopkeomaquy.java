 ...
 * @author Admin
 */
public class Tophopkeomaquy {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophopkeomaquy INSTANCE = new Tophopkeomaquy();

    public static Tophopkeomaquy getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hopkeomaquy, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hopkeomaquy IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hopkeomaquy = rs.getInt("hopkeomaquy");
