 ...
 * @author Admin
 */
public class Topbongmaster {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topbongmaster INSTANCE = new Topbongmaster();

    public static Topbongmaster getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.bong_master, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.bong_master IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.bongmaster = rs.getInt("bong_master");
