 ...
 * @author Admin
 */
public class Topcapsuvip {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topcapsuvip INSTANCE = new Topcapsuvip();

    public static Topcapsuvip getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.capsuvip, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.capsuvip IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.capsuvip = rs.getInt("capsuvip");
