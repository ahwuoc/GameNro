 ...
 * @author Admin
 */
public class Tophoptrahoacuc {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Tophoptrahoacuc INSTANCE = new Tophoptrahoacuc();

    public static Tophoptrahoacuc getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.hoptrahoacuc, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.hoptrahoacuc IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.hoptrahoacuc = rs.getInt("hoptrahoacuc");
