 ...
 * @author Admin
 */
public class Topvongquayvang {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Topvongquayvang INSTANCE = new Topvongquayvang();

    public static Topvongquayvang getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.vongquayvang, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.vongquayvang IS NOT NULL " +
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.vongquayvang = rs.getInt("vongquayvang");
