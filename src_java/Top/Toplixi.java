 ...
 * @author Admin
 */
public class Toplixi {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final Toplixi INSTANCE = new Toplixi();

    public static Toplixi getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = "SELECT p.id, p.name, p.head, p.gender, p.data_inventory, " +
             "p.data_point, p.items_body, a.lixi, p.firstTimeLogin " +
             "FROM player p " +
             "JOIN account a ON p.account_id = a.id " +
             "WHERE a.lixi IS NOT NULL " +
