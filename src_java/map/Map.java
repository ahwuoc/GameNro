...
public class Map implements Runnable {
    ...
    public List<Npc> npc_list_create;
    ...
    public void initNpc(byte[] npcId, short[] npcX, short[] npcY) {
        this.npc_list_create = new ArrayList<>();
        for (int i = 0; i < npcId.length; i++) {
            this.npc_list_create.add(NpcFactory.createNPC(this.mapId, 1, npcX[i], npcY[i], npcId[i]));
        }
    }
    public void addNpc(Npc npc) {
        this.npc_list_create.add(npc);
    }
   @Override
    public void run() {
        while (true) {
