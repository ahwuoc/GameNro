...
import models.Combine.manifest.HacHoaPorata;
import models.Combine.manifest.HoiPhucSach;
import models.Combine.manifest.MoChiSoLinhThu;
import models.Combine.manifest.MoChiSoOption1;
import models.Combine.manifest.MoChiSoOption2;
import models.Combine.manifest.MoKhoaItem;
import models.Combine.manifest.NangCapBongTai;
import models.Combine.manifest.NangCapBongTai2;
...
import models.Combine.manifest.PhanRaDoTl;
...
public class CombineService {
    ...
    public static final int NANG_CAP_BONG_TAI2 = 535;
    public static final int MO_CHI_SO_OPTION1 = 536;
    public static final int HAC_HOA_PORATA = 538;
    public static final int MO_CHI_SO_OPTION2 = 537;
    ...
    public static final int PHAN_RA_DO_TL = 52323;
    ...
    public void showInfoCombine(Player player, int[] index) {
        ...
        if (index.length > 0) {
            for (int i = 0; i < index.length; i++) {
                player.combine.itemsCombine.add(
                    player.inventory.itemsBag.get(index[i])
                );
            }
        }
        switch (player.combine.typeCombine) {
            case PHAN_RA_DO_TL -> PhanRaDoTl.showInfoCombine(player);
            case TIEN_CAP_BROLY -> TienCapBroly.showInfoCombine(player);
            case TIEN_CAP_VEGETA -> TienCapVegeta.showInfoCombine(player);
            case TIEN_CAP_NAMEC -> TienCapNamec.showInfoCombine(player);
            case TIEN_CAP_TRUNK -> TienCapTrunk.showInfoCombine(player);
            case NANG_CAP_LINH_THU -> TienCapLinhThu.showInfoCombine(player);
            case PHAN_RA_LINH_THU -> PhanRaLinhThu.showInfoCombine(player);
            case MO_CHI_SO_LINH_THU -> MoChiSoLinhThu.showInfoCombine(player);
            case PHAN_RA_X1 -> PhaRaX1.showInfoCombine(player);
            ...
            case NANG_CAP_BONG_TAI2 -> NangCapBongTai2.showInfoCombine(player);
            case MO_CHI_SO_OPTION1 -> MoChiSoOption1.showInfoCombine(player);
            case HAC_HOA_PORATA -> HacHoaPorata.showInfoCombine(player);
            case MO_CHI_SO_OPTION2 -> MoChiSoOption2.showInfoCombine(player);
            case NANG_CHI_SO_BONG_TAI -> NangChiSoBongTai.showInfoCombine(
                player
            );
            case NANG_CAP_SAO_PHA_LE -> NangCapSaoPhaLe.showInfoCombine(player);
            case CUONG_HOA_LO_SAO_PHA_LE -> CuongHoaLoSaoPhaLe.showInfoCombine(
                player
            );
            case GIAM_DINH_SACH -> GiamDinhSach.showInfoCombine(player);
            case TAY_SACH -> TaySach.showInfoCombine(player);
            case NANG_CAP_SACH_TUYET_KY -> NangCapSachTuyetKy.showInfoCombine(
                player
            );
            case HOI_PHUC_SACH -> HoiPhucSach.showInfoCombine(player);
            case PHAN_RA_SACH -> PhanRaSach.showInfoCombine(player);
            case CHE_TAO_TRANG_BI_THIEN_SU -> CheTaoTrangBiThienSu.showInfoCombine(
                player
            );
            case NANG_CAP_KICH_HOAT -> NangCapKichHoat.showInfoCombine(player);
            case NANG_CAP_KICH_HOAT_VIP -> NangCapKichHoatVip.showInfoCombine(
                player
            );
            case DAP_DO_AO_HOA -> DapDoAoHoa.showInfoCombine(player);
            case PS_HOA_TRANG_BI -> PhapSuHoa.showInfoCombine(player);
            case TAY_PS_HOA_TRANG_BI -> RemoveOptionItem.showInfoCombine(
                player
            );
            case MO_KHOA_ITEM -> MoKhoaItem.showInfoCombine(player);
            case NANG_CAP_CHAN_MENH -> NangCapChanMenh.showInfoCombine(player);
            case AN_TRANG_BI -> TinhAnTrangBi.showInfoCombine(player);
            case GIA_HAN_VAT_PHAM -> GiaHanVatPham.showInfoCombine(player);
            case SIEU_HOA -> SieuHoaCaiTrang.showInfoCombine(player);
            case TINH_THACH_HOA -> TinhThachHoa.showInfoCombine(player);
            case NANG_GIAP_LUYEN_TAP -> NangGiapLuyenTap.showInfoCombine(
                player
            );
        }
    }
    ...
    public void startCombine(Player player, int... n) {
        ...
        switch (player.combine.typeCombine) {
            case TIEN_CAP_VEGETA -> TienCapVegeta.startCombine(player);
            case TIEN_CAP_NAMEC -> TienCapNamec.startCombine(player);
            case TIEN_CAP_BROLY -> TienCapBroly.startCombine(player);
            case TIEN_CAP_TRUNK -> TienCapTrunk.startCombine(player);
            case NANG_CAP_LINH_THU -> TienCapLinhThu.startCombine(player);
            case PHAN_RA_LINH_THU -> PhanRaLinhThu.startCombine(player);
            case MO_CHI_SO_LINH_THU -> MoChiSoLinhThu.startCombine(player);
            //
            case EP_SAO_TRANG_BI -> EpSaoTrangBi.epSaoTrangBi(player);
            case PHA_LE_HOA_TRANG_BI -> PhaLeHoaTrangBi.phaLeHoa(player, num);
            case NHAP_NGOC_RONG -> NhapNgocRong.nhapNgocRong(player, num == 1);
            case NANG_CAP_VAT_PHAM -> NangCapVatPham.nangCapVatPham(
                player,
                num == 1
            );
            case NANG_CAP_BONG_TAI -> NangCapBongTai.nangCapBongTai(player);
            case NANG_CAP_BONG_TAI2 -> NangCapBongTai2.nangCapBongTai(player);
            case NANG_CHI_SO_BONG_TAI -> NangChiSoBongTai.nangChiSoBongTai(
                player
            );
            case MO_CHI_SO_OPTION1 -> MoChiSoOption1.nangChiSoBongTai(player);
            case HAC_HOA_PORATA -> HacHoaPorata.nangCapBongTai(player);
            case MO_CHI_SO_OPTION2 -> MoChiSoOption2.nangChiSoBongTai(player);
            case NANG_CAP_SAO_PHA_LE -> NangCapSaoPhaLe.nangCapSaoPhaLe(player);
            case CUONG_HOA_LO_SAO_PHA_LE -> CuongHoaLoSaoPhaLe.cuongHoaLoSaoPhaLe(
                player
            );
            case GIAM_DINH_SACH -> GiamDinhSach.giamDinhSach(player);
            case TAY_SACH -> TaySach.taySach(player);
            case PHAN_RA_X1 -> PhaRaX1.startCombine(player);
            case PHAN_RA_DO_TL -> PhanRaDoTl.startCombine(player);
            case NANG_CAP_SACH_TUYET_KY -> NangCapSachTuyetKy.nangCapSachTuyetKy(
                player
            );
            case HOI_PHUC_SACH -> HoiPhucSach.hoiPhucSach(player);
            case PHAN_RA_SACH -> PhanRaSach.phanRaSach(player);
            case CHE_TAO_TRANG_BI_THIEN_SU -> CheTaoTrangBiThienSu.cheTaoTrangBiThienSu(
                player
            );
            case NANG_CAP_KICH_HOAT -> NangCapKichHoat.startCombineNew(player);
            case NANG_CAP_KICH_HOAT_VIP -> NangCapKichHoatVip.startCombine(
                player
            );
            case DAP_DO_AO_HOA -> DapDoAoHoa.startCombine(player);
            case PS_HOA_TRANG_BI -> PhapSuHoa.startCombine(player);
            case TAY_PS_HOA_TRANG_BI -> RemoveOptionItem.startCombine(player);
            case MO_KHOA_ITEM -> MoKhoaItem.startCombine(player);
            case NANG_CAP_CHAN_MENH -> NangCapChanMenh.startCombine(player);
            case AN_TRANG_BI -> TinhAnTrangBi.startCombine(player);
            case GIA_HAN_VAT_PHAM -> GiaHanVatPham.startCombine(player);
            case SIEU_HOA -> SieuHoaCaiTrang.startCombine(player);
            case TINH_THACH_HOA -> TinhThachHoa.startCombine(player);
            case NANG_GIAP_LUYEN_TAP -> NangGiapLuyenTap.startCombine(player);
        }
    ...
    public void sendEffectCombineItem(
        Player player,
        byte type,
        short icon1,
        short icon2
    ...
    public void sendAddItemCombine(Player player, int npcId, Item... items) {
        ...
        try {
            msg = new Message(-81);
            msg.writer().writeByte(0);
            msg.writer().writeUTF("By DucPro");
            msg.writer().writeUTF("DucPro - 0822992003");
    ...
    private String getTextTopTabCombine(int type) {
        return switch (type) {
            case TIEN_CAP_VEGETA -> "Ta sẽ giúp ngươi\n tiến cấp cải trang Vegeta \n Cải tràn chỉ số cao hơn";
            case TIEN_CAP_NAMEC -> "Ta sẽ giúp ngươi\n tiến cấp cải trang namec \n Cải tràn chỉ số cao hơn";
            case TIEN_CAP_TRUNK -> "Ta sẽ giúp ngươi\n tiến cấp cải trang Trunk \n Cải tràn chỉ số cao hơn";
            case TIEN_CAP_BROLY -> "Ta sẽ giúp ngươi\n tiến cấp cải trang BROLY \n Cải tràn chỉ số cao hơn";
            //
            case NANG_CAP_CHAN_MENH -> "Ta sẽ phù phép\ncho cHÂN MỆNH của ngươi\n Đạt lever cao hơn!!";
            case EP_SAO_TRANG_BI -> "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở nên mạnh mẽ";
            case PHA_LE_HOA_TRANG_BI -> "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở thành trang bị pha lê";
            case
                CHUYEN_HOA_TRANG_BI_DUNG_VANG,
                CHUYEN_HOA_TRANG_BI_DUNG_NGOC -> "Lưu ý trang bị mới\nphải hơn trang bị gốc\n1 bậc";
            case NHAP_NGOC_RONG -> "Ta sẽ phù phép\ncho 7 viên Ngọc Rồng\nthành 1 viên Ngọc Rồng cấp cao";
            case PHAN_RA_X1 -> "Ta sẽ giúp ngươi phân rã\n đồ thần linh thành thỏi vàng";
            case PHAN_RA_DO_TL -> "Ta sẽ giúp ngươi phân rã\n đồ thần linh để nhận 100 Ruby";
            case NHAP_DA -> "Ta sẽ phù phép\ncho 10 mảnh đá vụn\ntrở thành 1 đá nâng cấp";
            case NANG_CAP_VAT_PHAM -> "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở nên mạnh mẽ";
            case NANG_CAP_BONG_TAI -> "Ta sẽ phù phép\ncho bông tai Porata của ngươi\nthành cấp 2";
            case NANG_CAP_BONG_TAI2 -> "Ta sẽ phù phép\ncho bông tai Porata cấp 2 của ngươi\nthành cấp 3";
            case NANG_CHI_SO_BONG_TAI -> "Ta sẽ phù phép\ncho bông tai Porata cấp 2 của ngươi\ncó 1 chỉ số ngẫu nhiên";
            case
                MO_CHI_SO_OPTION1,
                MO_CHI_SO_OPTION2 -> "Ta sẽ phù phép\ncho bông tai Porata cấp 3 hoặc bông tai hắc hóa của ngươi\ncó chỉ số  1 ngẫu nhiên";
            case HAC_HOA_PORATA -> "Ta sẽ Hắc hóa porata3\n trở thành porata Hắc Hóa";
            case NANG_CAP_SAO_PHA_LE -> "Ta sẽ phù phép\nnâng cấp Sao Pha Lê\nthành cấp 2";
            case DANH_BONG_SAO_PHA_LE -> "Đánh bóng\nSao pha lê cấp 2";
            case CUONG_HOA_LO_SAO_PHA_LE -> "Cường hóa\nÔ Sao Pha Lê";
            case TAO_DA_HEMATITE -> "Ta sẽ phù phép\ntạo đá hematite";
            case GIAM_DINH_SACH -> "Ta sẽ phù phép\ngiám định sách đó cho ngươi";
            case TAY_SACH -> "Ta sẽ phù phép\ntẩy sách đó cho ngươi";
            case NANG_CAP_SACH_TUYET_KY -> "Ta sẽ phù phép\nnâng cấp Sách Tuyệt Kỹ cho ngươi";
            case HOI_PHUC_SACH -> "Ta sẽ phù phép\nphục hồi sách cho ngươi";
            case PHAN_RA_SACH -> "Ta sẽ phù phép\nphân rã sách đó cho ngươi";
            case CHE_TAO_TRANG_BI_THIEN_SU -> "Chế tạo\ntrang bị thiên sứ";
            case LAM_PHEP_NHAP_DA -> "Ta sẽ phù phép\n" +
            "cho 10 mảnh đá vụn\n" +
            "trở thành 1 đá nâng cấp";
            case NANG_CAP_KICH_HOAT -> "Ta sẽ phù phép\nchế tạo trang bị Thần Linh\nthành trang bị Kích Hoạt";
            case NANG_CAP_KICH_HOAT_VIP -> "Ta sẽ phù phép\nchế tạo trang bị kích hoạt VIP";
            case GIA_HAN_VAT_PHAM -> "Ta sẽ phù phép\ncho trang bị của ngươi\nthêm hạn sử dụng";
            case SIEU_HOA -> "Ta sẽ giúp con siêu hóa\n Cải trang";
            case TINH_THACH_HOA -> "Ta sẽ giúp con Tinh Thạch đồ";
            case DAP_DO_AO_HOA -> "Ta sẽ giúp ngươi ảo hóa đồ để có thuộc tính cao hơn";
            case PS_HOA_TRANG_BI -> "Pháp sư hóa pet, linh thú, ván bay";
            case TAY_PS_HOA_TRANG_BI -> "Tẩy đồ";
            case MO_KHOA_ITEM -> "Mở Khóa giao dịch Item";
            case AN_TRANG_BI -> "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở thành trang bị Ấn";
            default -> "";
        };
    }
    ...
    private String getTextInfoTabCombine(int type) {
        return switch (type) {
            case TIEN_CAP_VEGETA -> "Vào hành trang\n" +
            "Chọn cải trang Vegeta\n" +
            "Chọn mảnh cải trang\n" +
            "Chọn thỏi vàng\n" +
            "Sau đó ấn nâng cấp";
            case TIEN_CAP_NAMEC -> "Vào hành trang\n" +
            "Chọn cải trang Namec\n" +
            "Chọn thỏi vàng\n" +
            "Chọn mảnh cải trang\n" +
            "Sau đó ấn nâng cấp";
            case TIEN_CAP_BROLY -> "Vào hành trang\n" +
            "Chọn cải trang broly\n" +
            "Chọn thỏi vàng\n" +
            "Chọn mảnh cải trang\n" +
            "Sau đó ấn nâng cấp";
            case TIEN_CAP_TRUNK -> "Vào hành trang\n" +
            "Chọn cải trang Trunk\n" +
            "Chọn thỏi vàng\n" +
            "Chọn mảnh cải trang\n" +
            "Sau đó ấn nâng cấp";
            case NANG_CAP_LINH_THU -> "Vào hành trang\n" +
            "Chọn LinhThu\n" +
            "Chọn thỏi vàng\n" +
            "Chọn Hồn Linh Thu\n" +
            "Sau đó ấn nâng cấp";
            case PHAN_RA_LINH_THU -> "Vào hành trang\n" +
            "Chọn LinhThu\n" +
            "Sau đó ấn Phân rã";
            case MO_CHI_SO_LINH_THU -> "Vào hành trang\n" +
            "Chọn Linh Thú cần mở chỉ số để đầu tiên\n" +
            "Chọn thỏi vàng\n" +
            "Linh Thú cùng cấp\n" +
            "Sau đó ấn Mở chỉ số";
            //
            case NANG_CAP_CHAN_MENH -> "vào hành trang chọn 1 chân mênh\n và đá hoàng kim\n sau đó ấn nâng cấp";
            case EP_SAO_TRANG_BI -> "Vào hành trang\nChọn trang bị\n(Áo, quần, găng, giày hoặc rađa) có ô đặt sao pha lê\nChọn loại sao pha lê\nSau đó chọn 'Nâng cấp'";
            case PHA_LE_HOA_TRANG_BI -> "Vào hành trang\nChọn trang bị\n(Áo, quần, găng, giày hoặc rađa)\nSau đó chọn 'Nâng cấp'";
            case
                CHUYEN_HOA_TRANG_BI_DUNG_VANG,
                CHUYEN_HOA_TRANG_BI_DUNG_NGOC -> "Vào hành trang\nChọn trang bị gốc\n(Áo,quần,găng,giày hoặc rađa)\ntừ cấp [+4] trở lên\nChọn tiếp trang bị mới\nchưa nâng cấp cần nhập thể\nsau đó chọn 'Nâng cấp'";
            case NHAP_NGOC_RONG -> "Vào hành trang\nChọn 7 viên ngọc cùng sao\nSau đó chọn 'Làm phép'";
            case NHAP_DA -> "Vào hành trang\nChọn 10 mảnh đá vụn\nChọn 1 bình nước phép\n(mua tại Uron ở trạm tàu vũ trụ)\nSau đó chọn 'Làm phép'";
            case NANG_CAP_VAT_PHAM -> "Vào hành trang\nChọn trang bị\n(Áo,quần,găng,giày hoặc rađa)\nChọn loại đá để nâng cấp\nSau đó chọn 'Nâng cấp'";
            case NANG_CAP_BONG_TAI -> "Vào hành trang\nChọn bông tai Porata\nChọn mảnh bông tai để nâng cấp, số lượng 9999 cái\nSau đó chọn 'Nâng cấp'";
            case NANG_CAP_BONG_TAI2 -> "Vào hành trang\nChọn bông tai Porata cấp 2\nChọn mảnh bông tai 3 để nâng cấp, số lượng 9999 cái\nSau đó chọn 'Nâng cấp'";
            case NANG_CHI_SO_BONG_TAI -> "Vào hành trang\nChọn bông tai Porata\nChọn mảnh hồn porata số lượng 99\ncái và đá xanh lam để nâng cấp.\nSau đó chọn 'Nâng cấp chỉ số'";
            case
                MO_CHI_SO_OPTION1,
                MO_CHI_SO_OPTION2 -> "Vào hành trang\nChọn bông tai Porata\nChọn mảnh hồn bông tai cấp 2(Nếu đổi dòng 1)\nHoặc mảnh hồn bông tai cấp 3(nếu đổi dòng 2)\nSố lượng x99 và đá xanh lam\nSau đó chọn 'Nâng cấp chỉ số'";
            case HAC_HOA_PORATA -> "Vào hành trang\n Chọn x99 Đá Hắc Hóa\n Bông tai (Cấp 3)\n sau đó bấm 'Hắc Hóa'";
            ...
            case CUONG_HOA_LO_SAO_PHA_LE -> "Vào hành trang\nChọn trang bị có Ô sao thứ 8 trở lên chưa cường hóa\nChọn đá Hematite\nChọn dùi đục\nSau đó chọn 'Cường hóa'";
            case TAO_DA_HEMATITE -> "Vào hành trang\nChọn 5 sao pha lê cấp 2 cùng màu\nChọn 'Tạo đá Hematite'";
            case GIAM_DINH_SACH -> "Vào hành trang chọn\n1 sách cần giám định";
            case TAY_SACH -> "Vào hành trang chọn\n1 sách cần tẩy";
            case NANG_CAP_SACH_TUYET_KY -> "Vào hành trang chọn\nSách Tuyệt Kỹ 1 cần nâng cấp và 10 Kìm bấm giấy";
            case PHAN_RA_X1 -> "Vào hành trang chọn\n 1 Đồ Thần Linh sau đó bấm phân rã ";
            case PHAN_RA_DO_TL -> "Vào hành trang chọn\n 1 Đồ Thần Linh sau đó bấm phân rã để nhận 100 Ruby";
            case HOI_PHUC_SACH -> "Vào hành trang chọn\nCác Sách Tuyệt Kỹ cần phục hồi";
            case PHAN_RA_SACH -> "Vào hành trang chọn\n1 sách cần phân rã";
            case CHE_TAO_TRANG_BI_THIEN_SU -> "Cần 1 công thức\nMảnh trang bị tương ứng\n1 đá nâng cấp (tùy chọn)\n1 đá may mắn (tùy chọn)";
            case LAM_PHEP_NHAP_DA -> "Vào hành trang\n" +
            "Chọn 10 mảnh đá vụn\n" +
            "Chọn 1 bình nước phép\n" +
            "(mua tại Uron ở trạm tàu vũ trụ)\n" +
            "Sau đó chọn 'Làm phép'";
            case NANG_CAP_KICH_HOAT -> "Vào hành trang\nChọn 3 trang bị Thần Linh\n\nSau đó chọn 'Nâng cấp'\n ngươi sẽ nhận được ngẫu nhiên 1 trang bị kích hoạt\n có thể nhận được đồ thần linh kích hoạt\n" +
            "Lưu ý đồ nhận được sẽ cùng với đồ đầu tiên bỏ vào";
            case NANG_CAP_KICH_HOAT_VIP -> "Từ 1tỉ5 lên đồ tl cần đồ kích hoạt 1tỉ5 + 150tv + 5viên đá nâng cấp kích hoạt + 5viên đá ngũ sắc +5 cỏ 4 lá \n" +
            "Tỉ lệ: 50% \n" +
