        ...
        if (player.combine != null && player.combine.itemsCombine != null && player.combine.itemsCombine.size() == 1) {
            ...
            if (trangBi!=null) {
               CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE,
                        "Phân Rã đồ thần linh để nhận 100 Ruby","Phân rã");
            } else {
    ...
    public static void startCombine(Player player) {
        if (player.combine != null && player.combine.itemsCombine != null && player.combine.itemsCombine.size() == 1) {
            ...
            if (isDoThanLinh(trangBi)) {
                ...
                Service.gI().sendThongBaoOK(player, "Bạn đã phân rã 1 đồ thần linh và nhận được 100 Ruby!");
