...
public class MoChiSoOption2 {
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (bongTai == null || manhHonBongTai == null || daXanhLam == null) {
            Service.gI().sendDialogMessage(player, "Cần 1 bông tai cấp 3 hoặc bông tai hắc hóa, 99 mảnh hồn porata 3 và 1 đá xanh lam.");
            return;
        }
        ...
        text.append(manhHonBongTai.quantity >= 99 ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Cần 99 Mảnh hồn bông tai 3\n");
        ...
        if (manhHonBongTai.quantity < 99) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, text.toString(), "Còn thiếu\n" + (99 - manhHonBongTai.quantity) + " Mảnh hồn bông tai 3");
            return;
        }
    ...
    public static void nangChiSoBongTai(Player player) {
        ...
        if (Util.isTrue(50, 100)) {
            ...
            int param2 = (Util.isTrue(8,10)? Util.nextInt(1,10): Util.nextInt(10, 15));
           for(int i =0;i<5;i++){
              if(i==2){
                   bongTai.itemOptions.add(new Item.ItemOption(option2, param2));
              }else{
                   bongTai.itemOptions.add(new Item.ItemOption(option[i], param[i]));
              }
           }
