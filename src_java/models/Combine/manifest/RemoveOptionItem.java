...
public class RemoveOptionItem {
    ...
    public static void showInfoCombine(Player player) {
        if (InventoryService.gI().getCountEmptyBag(player) > 0) {
            if (player.combine.itemsCombine.size() == 2) {
                ...
                if (daHacHoa == null) {
                    CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn còn thiếu ngọc tẩy",
                            "Đóng");
                    return;
                }
                if (itemHacHoa == null) {
    ...
    public static void startCombine(Player player) {
        ...
        if (player.combine.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isTrangBiHacHoa())
                .count() != 1) {
            return;
        }
        if (player.combine.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1708)
                .count() != 1) {
            return;
        }
        if (InventoryService.gI().getCountEmptyBag(player) > 0) {
            ...
            Item buagiaihachoa = player.combine.itemsCombine.stream().filter(item -> item.template.id == 1708)
                    .findFirst().get();
            ...
            if (Util.isTrue(100, 100)) {
                ...
                ItemOption option_57 = new ItemOption();
                ...
                for (ItemOption itopt : trangBiHacHoa.itemOptions) {
                    ...
                    } else if (trangBiHacHoa.template.type == 5 || trangBiHacHoa.template.type == 32
                            || trangBiHacHoa.template.type == 21 || trangBiHacHoa.template.type == 23
                            || trangBiHacHoa.template.type == 24 || trangBiHacHoa.template.type == 72
