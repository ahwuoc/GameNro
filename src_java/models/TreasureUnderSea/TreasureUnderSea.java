    ...
    private void init() {
        ...
        for (Zone zone : this.zones) {
            ...
            if (zone.map.mapId == 135 || zone.map.mapId == 136 || zone.map.mapId == 137) {
                ...
                for (int i = 0; i < mobs.size(); i++) {
                    ...
                    if (((i == 5 || i == 10) && zone.map.mapId == 135) || (i == 5 && zone.map.mapId == 136)
                        ...
                        mob.lvMob = 1;
                        mob.point.dame = Util.toIntOrLong(level * 31 * 50 * mob.tempId * 10);
                        mob.point.maxHp = Util.toIntOrLong(level * 3107 * 50 * mob.tempId * 10);
                        mob.hoiSinh();
                        mob.hoiSinhMobPhoBan();
                    } else {
                        mob.lvMob = 0;
                        mob.point.dame = Util.toIntOrLong(level * 31 * 50 * mob.tempId);
                        mob.point.maxHp = Util.toIntOrLong(level * 3107 * 50 * mob.tempId);
                        mob.hoiSinh();
                        mob.hoiSinhMobPhoBan();
                    }
                }
            ...
            if (zone.map.mapId == 137) {
                try {
                    long bossDamage = Util.toIntOrLong(200000 * level);
                    long bossMaxHealth = Util.toIntOrLong(200000000 * level);
