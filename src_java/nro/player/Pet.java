...
public class Pet extends Player {
    ...
    private static final short[][] PET_ID = {{285, 286, 287}, {288, 289, 290}, {282, 283, 284}, {304, 305, 303}, {946, 947, 948}, {1422, 1423, 1424}, {876, 877, 878}};
 ...
 public void fusion3(boolean porata3) {
    ...
    public void update() {
        try {
            ...
            // 2. Kiểm tra Zone của đệ tử
            if (this.zone == null || this.zone != this.master.zone) {
                joinMapMaster();
            }

            // QUAN TRỌNG: Nếu sau khi join mà vẫn không có zone thì return ngay
            if (this.zone == null) {
                return;
            }
            ...
            switch (status) {
                ...
                case ATTACK:
                    ...
                    // --- ƯU TIÊN ĐÁNH NGƯỜI (PK) ---
                    playerAttack = findPlayerAttack();
                    if (playerAttack != null) {
                        // Logic Hakai (Berus)
                        if (this.typePet == 9 && Util.isTrue(1, 5)
                                && playerAttack.nPoint.hp < 20_000_000
                        ...
                        // Tấn công người
                        handlePetAttack(playerAttack, null);
                        return;
                    }

                    // --- NẾU KHÔNG CÓ NGƯỜI, ĐÁNH QUÁI ---
                    mobAttack = findMobAttack();
                    if (mobAttack != null) {
                        handlePetAttack(null, mobAttack);
                    } else {
                        idle = true;
                    }
                    break;
                ...
                case GOHOME:
                    // Đã sửa lại đoạn này để tránh lỗi type byte/int
                    if (this.zone != null && (this.zone.map.mapId == 21 || this.zone.map.mapId == 22 || this.zone.map.mapId == 23)) {
                        if (System.currentTimeMillis() - lastTimeMoveAtHome > 5000) {
                            if (this.zone.map.mapId == 21 || this.zone.map.mapId == 23) {
                                if (directAtHome == -1) {
    ...
    // --- HÀM HỖ TRỢ TẤN CÔNG (Copy đoạn này bỏ xuống dưới cùng file Pet.java) ---
    private void handlePetAttack(Player plAttack, Mob mobAttack) {
        int dist = 0;
        int xTarget = 0;
        int yTarget = 0;

        if (plAttack != null) {
            dist = Util.getDistance(this, plAttack);
            xTarget = plAttack.location.x;
            yTarget = plAttack.location.y;
        } else if (mobAttack != null) {
            dist = Util.getDistance(this, mobAttack);
            xTarget = mobAttack.location.x;
            yTarget = mobAttack.location.y;
        } else {
            return;
        }

        // Kiểm tra khoảng cách để chọn skill
        if (dist <= ARANGE_ATT_SKILL1) {
            // Đấm (Skill 1)
            ...
            if (SkillService.gI().canUseSkillWithCooldown(this) && canAttack()) {
                if (SkillService.gI().canUseSkillWithMana(this)) {
                    // Di chuyển lại gần chút cho tự nhiên
                    PlayerService.gI().playerMove(this, xTarget + Util.nextInt(-60, 60), yTarget);
                    SkillService.gI().useSkill(this, plAttack, mobAttack, -1, null);
                } else {
                    askPea();
                }
            }
            ...
            // Nếu không có skill 2 hoặc chưa hồi thì dùng skill 1
            if (this.playerSkill.skillSelect.skillId != -1) {
                ...
                // Quay về skill 1 nếu không chưởng được
                this.playerSkill.skillSelect = getSkill(1);
                if (SkillService.gI().canUseSkillWithCooldown(this) && canAttack()) {
                    if (SkillService.gI().canUseSkillWithMana(this)) {
                        PlayerService.gI().playerMove(this, xTarget + Util.nextInt(-60, 60), yTarget);
