...
public class Archivement {
    ...
    public static void loadMocNapCache() {
        MOC_NAP_CACHE.clear();
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement("SELECT id, detail FROM moc_nap"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                int id = rs.getInt("id");
    ...
    /**
     * Nhận quà mốc nạp (ưu tiên cache, fallback SQL nếu cache trống)
     */
