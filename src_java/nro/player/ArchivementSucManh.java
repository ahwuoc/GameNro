...
public class ArchivementSucManh {
    ...
    private void nhanQua(Player pl, int index) {
        Item item;
        JSONArray dataArray = MOC_SUC_MANH_CACHE.get(index);
        if (dataArray == null) {
            // fallback: đọc trực tiếp SQL nếu chưa có trong cache
            try (Connection con2 = DBConnecter.getConnectionServer();
                 PreparedStatement ps = con2.prepareStatement("SELECT detail FROM moc_suc_manh WHERE id = ?")) {
                ps.setInt(1, index);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        dataArray = (JSONArray) JSONValue.parse(rs.getString("detail"));
                        if (dataArray != null) MOC_SUC_MANH_CACHE.put(index, dataArray);
                    }
                }
        ...
        if (dataArray == null || dataArray.isEmpty()) {
            Service.gI().sendThongBao(pl, "Không có dữ liệu quà mốc sức mạnh " + index);
            return;
        }
            ...
            for (Object obj : dataArray) {
                ...
                if (optionsArray != null) {
                    for (Object opt : optionsArray) {
                        JSONObject optionObject = (JSONObject) JSONValue.parse(String.valueOf(opt));
...
//                                    achievement.setInfo1("Mốc SM ");
    ...
    private static final Map<Integer, JSONArray> MOC_SUC_MANH_CACHE = new HashMap<>();
    public static void loadMocSucManhCache() {
        MOC_SUC_MANH_CACHE.clear();
        try (Connection con = DBConnecter.getConnectionServer();
             PreparedStatement ps = con.prepareStatement("SELECT id, detail FROM moc_suc_manh");
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    MOC_SUC_MANH_CACHE.put(id, dataArray);
                } catch (Exception e) {
                    e.printStackTrace();
                    MOC_SUC_MANH_CACHE.put(id, new JSONArray());
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
