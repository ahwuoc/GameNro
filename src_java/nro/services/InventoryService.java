...
public class InventoryService {
    ...
    public Item findVeTangNgoc(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 718)) {
                return item;
            }
        }
        return null;
...
//                break;
    ...
    public void itemBodyToBag(Player player, int index) {
        if (index > 13) {
            ...
            System.err.println(
                    "[" + java.time.LocalDateTime.now() + "] Slot " + index + " - Inventory is full"
            );
    ...
    public Item finditemLyNuocMia(Player player, int soluong) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1614 || item.template.id == 1615) && item.quantity >= soluong) {
                return item;
            }
        }
        return null;
    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                ...
                boolean isOptionMatch = checkListsEqual(it.itemOptions, itemAdd.itemOptions);
                if (!isOptionMatch && itemAdd.template.id != 2074 && !itemAdd.isDaNangCap()) {
                    continue;
                }

                int maxStack = 999;
                if (isSpecialStackItem(itemAdd.template.id)) {
                    maxStack = 2_000_000_000;
                }

                int space = maxStack - it.quantity;
                if (space > 0) {
                    int add = Math.min(itemAdd.quantity, space);
                    it.quantity += add;
    ...
    //tăng giới hạn item >999
    private boolean isSpecialStackItem(int id) {
        return id == 1705
                || id == 457
                || id == 1229
                || id == 1281
    ...
    public boolean findItemBongTaiCap3(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 1810) {
                return true;
            }
        }
        for (Item item : player.inventory.itemsBox) {
    ...
    public boolean findItemManhTuanLoc(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 2003) {
                return true;
            }
        }
        return false;
    ...
    public Item finditemnguyenlieuDen(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1311) && item.quantity >= 1) {
                return item;
            }
        }
        return null;
    ...
    public Item finditemnguyenlieucarot(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 670) && item.quantity >= 1) {
                return item;
            }
        }
        return null;
    ...
    public boolean addItemGift(Player player, Item item) {
        return addItemGift(player.inventory.itemsBoxCrackBall, item);
    }

    public boolean addItemGift(List<Item> items, Item itemAdd) {
        //nếu item ko có option, add option rỗng vào
        if (itemAdd.itemOptions.isEmpty()) {
            itemAdd.itemOptions.add(new ItemOption(73, 0));
        }
        //add item vào ô mới
        items.add(ItemService.gI().copyItemGift(itemAdd));
        itemAdd.quantity = 0;
        return true;
    }
    ...
    public boolean findItemNguoituyet(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 1448) {
                return true;
            }
        }
        return false;
    ...
    public boolean findItemNguoituyetbanggia(Player player) {
        for (Item item : player.inventory.itemsBag) {
            if (item.isNotNullItem() && item.template.id == 1449) {
                return true;
            }
        }
        return false;
