...
public class TaskService {
    ...
    public void checkDoneTaskJoinClan(Player player) {
        if (!player.isBoss && !player.isPet && !player.isBot && !player.isClone) {
            doneTask(player, ConstTask.TASK_12_0);
        }
    }
    ...
    public void checkDoneTaskPickItem(Player player, ItemMap item) {
        if (!player.isBoss && !player.isPet && !player.isBot && !player.isClone && item != null) {
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet && !player.isBot && !player.isClone) {
            switch ((int) boss.id) {
                case BossID.KUKU:
    ...
    public int getIdTask(Player player) {
        if (player.isPet && !player.isBot && !player.isClone || player.isBoss || player.playerTask == null || player.playerTask.taskMain == null) {
            return -1;
        }
    ...
    public void addExpiryTime(Item item, String dateEnd) {
        try {
            if (item == null) {
                return;
            }
            ...
            // Tính thời gian hết hạn (đổi sang giây)
            long expiry = TimeUtil.getTime(dateEnd, "dd-MM-yyyy");
            int expirySeconds = (int) (expiry / 1000);

            // Kiểm tra xem item đã có option 73 (time) chưa
            boolean hasExpiry = item.itemOptions.stream()
                    .anyMatch(opt -> opt.optionTemplate.id == 73);

            if (!hasExpiry) {
                item.itemOptions.add(new Item.ItemOption(73, expirySeconds));
            } else {
                // Nếu đã có, cập nhật lại (tránh trùng)
                for (Item.ItemOption opt : item.itemOptions) {
                    if (opt.optionTemplate.id == 73) {
                        opt.param = expirySeconds;
                        break;
                    }
                }
            }
    ...
    public void paySideTask(Player player) {
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                Item quetre = ItemService.gI().createNewItem((short) 1366);
                int goldReward = 0;
                int ngocBi = 708;
                int cayThong = -1;
                switch (player.playerTask.sideTask.level) {
                    case ConstTask.EASY:
                        ...
                        if (EventManager.TEACHERS_DAY && quetre != null) {
                            quetre.quantity += 1;
                            addExpiryTime(quetre, "30-11-2025");
                            boolean hasExpiry = quetre.itemOptions.stream().anyMatch(opt -> opt.optionTemplate.id == 73);
                        }
                        ngocBi = 708;
