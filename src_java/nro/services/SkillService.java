...
import services.func.RadaService;
...
public class SkillService {
    ...
    public boolean useSkill(Player player, Player plTarget, Mob mobTarget, int status, Message msg) {
        if (player.clone != null) {
                useSkill(player.clone, plTarget, mobTarget, -1,null);
         }
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN: //kaioken
                ...
                if (Util.maxIntValue(player.nPoint.hp) <= hpUse) {
                    ...
                    Service.gI().sendEffAllPlayer(player, 1027, 1, 10, 10,1);
...
//                break;    
        ...
        if (plAtt != null) {
            ...
            if (percentPST != 0) {
                ...
                try {
                    ...
                    msg.writeLongByEmti(Util.maxIntValue(plAtt.nPoint.hp), ConstNpcConfig.readInt);
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        try {
            ...
            msg.writeLongByEmti(Util.maxIntValue(dameHit), ConstNpcConfig.readInt); //dame ăn
    ...
    private void playerAttackMob(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) {
        if (!mob.isDie() && plAtt != null && plAtt.nPoint != null && plAtt.playerSkill != null) {
            ...
            if ((plAtt.charms != null && plAtt.charms.BatTuHalloween > System.currentTimeMillis() 
                    || plAtt.itemTime.isMaTroi
                    || plAtt.itemTime.isBoXuong
                    || plAtt.itemTime.isDoiNhi
                    || plAtt.itemTime.isBiMa
                    ) && plAtt.nPoint.hp <= 1) {
                if (plAtt.nPoint.hp < 1) {
                    plAtt.nPoint.hp = 1;
                }
...
//            }
    ...
    private void sendPlayerAttackMob(Player plAtt, Mob mob) {
     ...
     public void selectSkill(Player player, int skillId) {// cần sữa
