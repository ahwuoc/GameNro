...
//                    break;
                ...
                case -66:
                    if (player != null) {
                        ...
                        if (idT == 25) {
                            idT = 51;
                            if (shenronType != -1 && player.zone.map.mapId != 0 && player.zone.map.mapId != 7 && player.zone.map.mapId != 14) {
                                idT = (shenronType == 1 || shenronType == 0) ? 59 : 60;
                            }
                        }
                ...
                case 34:
                    if (player != null) {
                        ...
                        if (selectSkill == Skill.BIEN_KHI || selectSkill == Skill.HUYT_SAO || selectSkill == Skill.BIEN_HINH) {
                            SkillService.gI().useSkill(player, null, null, -1, null);
                        }
                        if (selectSkill == Skill.PHAN_THAN) {
                ...
                case -118:
                    if (player != null) {
                        ...
                        switch (menuType) {
                            ...
                            case 21, 22, 23 -> {
                                ChuyenKhoanManager.HandleTransaction(player, _id);
                                break;
                            }
                            default -> {
                                if (player.isAdmin()) {
...
//                        byte index = _msg.reader().readByte();
                        if (player.zone != null && player.zone.map != null && player.typeRecvieArchiment == 2) {
                            ...
                            ArchivementSanBoss.gI().receiveGem(_msg.reader().readByte(), player);
                        }
...
//                            player.sendNewPet();
    ...
    public void sendInfo(MySession session) {
        try {
            ...
            DataGame.sendTileSetInfo(session);          // -82 set tile map
            IntrinsicService.gI().sendInfoIntrinsic(player); // 112 my info intrinsic
            Service.gI().point(player);                 // -42 my point
            TaskService.gI().sendTaskMain(player);      // 40 task
            Service.gI().clearMap(player);              // -22 reset all
            ClanService.gI().sendMyClan(player);        // -53 my clan
            PlayerService.gI().sendMaxStamina(player);  // -69 max stamina
            PlayerService.gI().sendCurrentStamina(player); // -68 cur stamina
            Service.gI().sendNangDong(player);          // -97 năng động
            Service.gI().sendHavePet(player);           // -107 have pet
            Service.gI().sendTopRank(player);           // -119 top rank

            // Cập nhật superRank nếu chưa có
            ...
            // Thông báo server
            ServerNotify.gI().sendNotifyTab(player);    // -50 thông tin bảng thông báo
...
//                    try {
//                        Thread.sleep(1000);
//                        Service.gI().sendPetFollow(
//                                player,
//                                (short) (player.inventory.itemsBody.get(13).template.iconID - 1)
//                        );
//                    } catch (Exception e) {
//                        e.printStackTrace();
//                    }
//                });
//            }
            // Spawn pet sau 1 giây bằng Virtual Thread
            if (player.inventory.itemsBody.get(7).isNotNullItem()) {
                Thread.ofVirtual().name("Pet update").start(() -> {
                    try {
                        Thread.sleep(1000);
