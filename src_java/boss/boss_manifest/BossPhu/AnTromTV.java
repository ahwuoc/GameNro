...
public class AnTromTV extends Boss {
    ...
    private boolean check(int[] nro, int sl){
       for(int i = 0;i<7;i++){
           if(nro[i]<sl) return true;
       }
       return false;
       
    }
    private int timnrotrom(int[]nro, int sl){
        List<Integer> list = new ArrayList<>();
        for(int i  = 0; i<7;i++){
            if(nro[i]<sl)list.add(i);
    ...
    public void attack() {
        if (Util.canDoWithTime(this.lastTimeAttack, 100) && this.typePk == ConstPlayer.PK_ALL) {
            ...
            try {
                ...
                if (!Util.canDoWithTime(this.lastTimeAnTrom, Util.nextInt(30000,60000))) {
                        return;
                    }
                ...
                if (thoivang == null) {
                 ...
                 if (Util.getDistance(this, pl) <= 40) {
                     if (pl.isPl()) {
                        InventoryService.gI().subQuantityItemsBag(pl, thoivang, 1);                      
                   ...
                   if (Util.isTrue(1, 2)) {
                    ...
                    int id = timnrotrom(nro,20);
                      Item ngocr = InventoryService.gI().findItemBag(pl, id);
                      if (ngocr == null) {
                       return;
                      }
                      if (Util.getDistance(this, pl) <= 40) {
                     if (pl.isPl()) {
                        InventoryService.gI().subQuantityItemsBag(pl, ngocr, 1);                      
                        InventoryService .gI().sendItemBag(pl);
                        InventoryService.gI().sendItemBag(this);
                        ItemMap itemMap = new ItemMap(this.zone, id, 1, (this.location.x + pl.location.x) / 2, this.location.y, this.id);
                        Service.gI().dropItemMap(this.zone, itemMap);
                        Service.gI().sendToAntherMePickItem(this, itemMap.itemMapId);
                        nro[id-14]++;
                        this.zone.removeItemMap(itemMap);
    ...
    public void reward(Player plKill) {
        ...
        for(int i = 0;i<7;i++){
            if(nro[i]>0){
                int sl = (int)nro[i]*80/100;
                ItemMap it = new ItemMap(this.zone, i+14, (int) sl, this.location.x + i * 10, this.zone.map.yPhysicInTop(this.location.x,
                        this.location.y - 24), -1);
