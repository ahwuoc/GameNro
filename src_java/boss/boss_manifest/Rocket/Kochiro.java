 ...
 */
public class Kochiro extends Boss {

    private long st;

    public Kochiro() throws Exception {
        super(BossID.KOCHIRO, BossesData.KOCHIRO);
    }
    ...
    public void reward(Player plKill) {
        ...
        if (Util.isTrue(90, 100)) {
            ItemMap caiTrang = new ItemMap(this.zone, 1876, 1, this.location.x + Util.nextInt(-15, 15),
                    this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
            caiTrang.options.add(new ItemOption(77, 38));
            caiTrang.options.add(new ItemOption(103, 38));
            caiTrang.options.add(new ItemOption(108, 15));
    ...
    public void doneChatE() {
        ...
        for (Boss boss : this.parentBoss.bossAppearTogether[this.parentBoss.currentLevel]) {
            if ((boss.id == BossID.MUSASHI) && !boss.isDie()) {
                return;
            }
        }
