...
public class ArchivementSucManh {
    ...
    public final static long POWER = 100000;
    public static long[] POWERGIFT = {POWER * 1, POWER * 10, POWER * 50, POWER * 100, POWER * 200, POWER * 500, POWER * 1000};
    ...
    public boolean checktongnap(Player pl, int index) {
        if (index == 0 && pl.lucchien >= POWERGIFT[0]) {
            return true;
        }
        if (index == 1 && pl.lucchien >= POWERGIFT[1]) {
            return true;
        }
        if (index == 2 && pl.lucchien >= POWERGIFT[2]) {
            return true;
        }
        if (index == 3 && pl.lucchien >= POWERGIFT[3]) {
            return true;
        }
        if (index == 4 && pl.lucchien >= POWERGIFT[4]) {
            return true;
        }
        if (index == 5 && pl.lucchien >= POWERGIFT[5]) {
            return true;
        }
        if (index == 6 && pl.lucchien >= POWERGIFT[6]) {
            return true;
        }
    ...
    public void getAchievement(Player player) {
        try {
            ...
            try (Connection con = DBConnecter.getConnectionServer();
                 PreparedStatement ps = con.prepareStatement("SELECT `Achievement_SucManh` FROM `player` WHERE id = ? LIMIT 1")) {
...
//                                    achievement.setInfo1("Mốc SM ");
                                    achievement.setInfo1("Mốc " + getNhiemVu(i) + " Lực Chiến");
                                    achievement.setInfo2("Đã đạt: " + getNhiemVu2(player, i) + "/" + getNhiemVu(i) + " Lực Chiến");
    ...
    public String getNhiemVu2(Player player, int index) {
        switch (index) {
            case 0:
                return " " + Util.numberToMoney(player.lucchien) + "";
            case 1:
                return " " + Util.numberToMoney(player.lucchien) + "";
            case 2:
                return " " + Util.numberToMoney(player.lucchien) + "";
