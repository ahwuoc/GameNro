...
/**
 * Quản lý sự kiện Top Thiệp Chúc VIP (Clean Code Version)
 */
public class ConstDataEventCHUCVIP {

    private static ConstDataEventCHUCVIP instance;

    public static ConstDataEventCHUCVIP gI() {
        if (instance == null) {
            instance = new ConstDataEventCHUCVIP();
        }
        return instance;
    }
    ...
    private static void traoQuaHangLoat() {
        Logger.log("Bắt đầu trao quà Top Thiệp Chúc VIP...");
        
        List<JSONArray> allRewards = loadAllRewards(); 
        if (allRewards.isEmpty()) {
            Logger.error("Không tìm thấy dữ liệu phần thưởng Top Thiệp Chúc VIP!");
            return;
        }

        List<Integer> topPlayerIds = getTopGreetingCardPlayerIds();
        ...
        for (int i = 0; i < topPlayerIds.size(); i++) {
            ...
            if (player != null) {
                ...
                Logger.log("Đã trao quà Top Thiệp Chúc VIP hạng " + (i + 1) + " cho: " + player.name);
                Functions.sleep(100); 
            } else {
                Logger.error("Không thể tải thông tin người chơi ID: " + playerId);
            }
        }
        Logger.log("Hoàn tất trao quà Top Thiệp Chúc VIP.");
    }

    private static List<Integer> getTopGreetingCardPlayerIds() {
        List<Integer> ids = new ArrayList<>();
        // Query tối ưu lấy Top dựa trên 'hopqua2010' (có vẻ là tên cột cho thiệp chúc)
        String sql = "SELECT player.id FROM account " +
                     "JOIN player ON account.id = player.account_id " +
                     "WHERE account.create_time > ? AND account.hopqua2010 >= 100000 " +
                     "ORDER BY account.hopqua2010 DESC LIMIT 10";
        ...
        try (Connection con = DBConnecter.getConnectionServer();
            ...
            Logger.error("Lỗi lấy danh sách Top Thiệp Chúc VIP: " + e.getMessage());
        }
        return ids;
    }
    ...
    private static List<JSONArray> loadAllRewards() {
        ...
        String sql = "SELECT detail FROM moc_hopqua2010 ORDER BY id ASC";
        ...
        try (Connection con = DBConnecter.getConnectionServer();
            ...
            Logger.error("Lỗi load phần thưởng Top Thiệp Chúc VIP: " + e.getMessage());
        }
        return rewards;
    }
    ...
    public static void traoQuaSuKien(Player pl, JSONArray rewardDetail) {
        ...
        try {
            ...
            if (NDVSqlFetcher.updateMailBox(pl)) {
                Service.gI().sendThongBao(pl, "Bạn vừa nhận thưởng Top Thiệp Chúc VIP về hòm thư!");
            } else {
                Logger.error("Lỗi lưu quà Top Thiệp Chúc VIP vào hòm thư cho: " + pl.name);
            }

        } catch (Exception e) {
            Logger.error("Lỗi xử lý quà Top Thiệp Chúc VIP cho " + pl.name + ": " + e.getMessage());
            e.printStackTrace();
        }
    }
}