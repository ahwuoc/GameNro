...
/**
 * Quản lý sự kiện Top 20/10 (Clean Code Version)
 */
public class ConstDataEventthangmuoi {

    private static ConstDataEventthangmuoi instance;

    public static ConstDataEventthangmuoi gI() {
        if (instance == null) {
            instance = new ConstDataEventthangmuoi();
        }
        return instance;
    }
    ...
    private static void initEventTime() {
        ...
        System.out.println("Start Event TOP 20/10: " + startEvent.getTime());
        System.out.println("End Event TOP 20/10: " + endEvents.getTime());
    }
    ...
    private static void traoQuaHangLoat() {
        Logger.log("Bắt đầu trao quà Top 20/10...");
        
        List<JSONArray> allRewards = loadAllRewards(); 
        if (allRewards.isEmpty()) {
            Logger.error("Không tìm thấy dữ liệu phần thưởng Top 20/10!");
            return;
        }

        List<Integer> topPlayerIds = getTop20_10PlayerIds();
        ...
        for (int i = 0; i < topPlayerIds.size(); i++) {
            ...
            if (player != null) {
                ...
                Logger.log("Đã trao quà Top 20/10 hạng " + (i + 1) + " cho: " + player.name);
                Functions.sleep(100); 
            } else {
                Logger.error("Không thể tải thông tin người chơi ID: " + playerId);
            }
        }
        Logger.log("Hoàn tất trao quà Top 20/10.");
    }

    private static List<Integer> getTop20_10PlayerIds() {
        List<Integer> ids = new ArrayList<>();
        // Query tối ưu lấy Top dựa trên 'thiepchucvip'
        String sql = "SELECT player.id FROM account " +
                     "JOIN player ON account.id = player.account_id " +
                     "WHERE account.create_time > ? AND account.thiepchucvip >= 100000 " +
                     "ORDER BY account.thiepchucvip DESC LIMIT 10";
        ...
        try (Connection con = DBConnecter.getConnectionServer();
            ...
            Logger.error("Lỗi lấy danh sách Top 20/10: " + e.getMessage());
        }
        return ids;
    }
    ...
    private static List<JSONArray> loadAllRewards() {
        ...
        String sql = "SELECT detail FROM moc_thiepchucvip ORDER BY id ASC";
        ...
        try (Connection con = DBConnecter.getConnectionServer();
            ...
            Logger.error("Lỗi load phần thưởng Top 20/10: " + e.getMessage());
        }
        return rewards;
    }
    ...
    public static void traoQuaSuKien(Player pl, JSONArray rewardDetail) {
        ...
        try {
            ...
            if (NDVSqlFetcher.updateMailBox(pl)) {
                Service.gI().sendThongBao(pl, "Bạn vừa nhận thưởng Top 20/10 về hòm thư!");
            } else {
                Logger.error("Lỗi lưu quà Top 20/10 vào hòm thư cho: " + pl.name);
            }

        } catch (Exception e) {
            Logger.error("Lỗi xử lý quà Top 20/10 cho " + pl.name + ": " + e.getMessage());
            e.printStackTrace();
        }
    }
}