...
impl MobService {
        ...
        if let Some(zone) = &player.zone {
            let msg_opt = {
                ...
                if let Some(mob) = mobs.iter_mut().find(|m| m.id == mob_id as u64) {
                    ...
                    if !mob.is_dead() {
                        Some(Self::build_mob_alive_message(
                            mob.id as i8,
                            new_hp,
                            real_damage,
                            false,
                        ...
                        Some(Self::build_mob_die_message(
                            mob.id as i8,
                            real_damage,
                            false,
                        ))
    ...
    pub async fn update(zone: &Zone) {
        let current_time = SystemTime::now()
            ...
            .unwrap()
            .as_millis() as u64;
        let mut msgs = Vec::new(); // Messages to be broadcast to all players
            ...
            for mob in mobs.iter_mut() {
                if !mob.is_alive {
                    if current_time > mob.last_time_die + 3000 {
                        ...
                        mob.hp = mob.max_hp;
                        mob.status = 5;
                        msgs.push(Self::build_mob_respawn_message(
                            mob.id as i8,
                            mob.template_id,
                            mob.hp,
                        ));
                    ...
                    if current_time > mob.last_time_attack_player + 2000 {
                        ...
                        if let Some(pid) = target_id {
                            if let Some(mut p_entry) = zone.players.get_mut(&pid) {
                                ...
                                let msg_me = Self::build_mob_attack_me_message(
                                    mob.id as i8,
                                    damage_taken as i32,
                                );
                                player_specific_msgs.push((player.id, msg_me));

                                let msg_other = Self::build_mob_attack_player_message(
                                    mob.id as i8,
                                    player.id as i32,
