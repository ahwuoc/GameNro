...
impl MobService {
    ...
    pub async fn create_mob(
        ...
        if let Some(ref database) = self.database {
            ...
            *next_id += 1;
            ...
            if let Ok(Some(mut mob)) =
                MobDao::create_mob_from_template(database, template_id, mob_id).await
            {
                ...
                println!(
                    "[MOB_SERVICE] Created mob {} (ID: {}) at ({}, {}) on map {}",
    ...
    pub async fn get_mobs_in_map(&self, map_id: i32, zone_id: i32) -> Vec<RtMob> {
        let mobs = self.mobs.read().await;
        mobs.values()
    ...
    pub async fn damage_mob(&self, mob_id: u64, damage: i32) -> bool {
        self.update_mob(mob_id, |mob| {
            ...
            println!(
                "[MOB_SERVICE] Mob {} (ID: {}) took {} damage, HP: {}/{}",
    ...
    pub async fn heal_mob(&self, mob_id: u64, amount: i32) -> bool {
        self.update_mob(mob_id, |mob| {
            mob.heal(amount);
            println!(
                "[MOB_SERVICE] Mob {} (ID: {}) healed {} HP, HP: {}/{}",
    ...
    pub async fn send_mob_info(
        ...
        println!(
            "[MOB_SERVICE] Sent mob info for {} (ID: {})",
            mob.name, mob.id
        );
        Ok(())
    }

    pub async fn send_mobs_in_map(
        &self,
        session: &mut AsyncSession,
        map_id: i32,
        zone_id: i32,
        ...
        println!(
            "[MOB_SERVICE] Sent {} mobs in map {} zone {}",
            mob_count, map_id, zone_id
        );
        Ok(())
    }
