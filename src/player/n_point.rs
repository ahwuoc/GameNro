...
impl NPoint {
    ...
    /// Tính lại toàn bộ chỉ số (gọi khi thay đổi trang bị, buff, ...)
    /// Tương đương với calPoint() trong Java
    pub fn cal_point(&mut self) {
        self.reset_bonus();
        // TODO: Duyệt items_body để cộng bonus từ item options
        // TODO: Duyệt buffs để cộng bonus
        self.set_base_point();
    }

    /// Reset tất cả bonus về 0 (trước khi tính lại)
    ...
    /// Tính các chỉ số cơ bản từ stats gốc + bonus
    /// Tương đương với setBasePoint() trong Java
    ...
    /// Set HP với giới hạn max
    pub fn set_hp(&mut self, value: i32) {
        self.hp = value.min(self.hp_max).max(0);
    }

    /// Set MP với giới hạn max
    pub fn set_mp(&mut self, value: i32) {
        self.mp = value.min(self.mp_max).max(0);
    }

    /// Thêm item option vào bonus (gọi khi duyệt trang bị)
    /// option_id: ID của option (từ item template)
