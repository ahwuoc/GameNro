...
impl PlayerInfoService {
    ...
    pub async fn send_task_info(session: &mut AsyncSession, player: &RtPlayer) -> anyhow::Result<()> {
        println!("Sending task main info for player {} (task_id={})", player.name, player.task_id);
        ...
        match task_main_id {
            0 => vec![
                SubTaskInfo {
                    ...
                    npc_id: if gender == 0 { 1 } else if gender == 1 { 2 } else { 3 },
